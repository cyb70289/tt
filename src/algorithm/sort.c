		...
		struct tt_sort_stat *stat)
{
	...
	for (int i = 1; i < input->count; i++) {
		p += input->size;
		/* tmp = &data[i] */
		input->__set(tmp, p, input->size);
		/* Insert data[i] into sorted list data[0] ~ data[i-1] */
		int j = i - 1;
		void *q = p - input->size;
		for (; j >= 0; q -= input->size, j--) {
			stat->time++;	/* Compare */
			/* if (*tmp < *q) */
			if (input->cmp(tmp, q) < 0) {
				input->__set(q + input->size, q, input->size);
				stat->time++;	/* Set */
			} else {
				break;
			}
		...
		/* data[j + 1] = *tmp */
		input->__set(q + input->size, tmp, input->size);
		stat->time++;	/* Set */
	}
...
int tt_sort(struct tt_sort_input *input)
{
	...
	input->__set = _tt_set_select(input->size);
